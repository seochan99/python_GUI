...
def add_file():
    files = filedialog.askopenfilenames(title="이미지 파일을 선택하세요",filetypes=(("PNG 파일","*.png"),("모든 파일","*.*")), \
        initialdir=r"C:/User/seochan/Downloads")

    #사용자가 선택한 파일 목록
    for file_ in files :
        list_file.insert(END,file_)
#del
def del_file():
    #뒤에서 부터 지워줘야한다.
...
def merge_image():
    ...
    dest_path = os.path.join(txt_dest_path.get(),"huichan_photo.jpg")
...
def start():
    ...
    #파일 목록 확인
    if list_file.size()==0:
        msgbox.showwarning("경고","이미지 파일 추가해세요..")
        return 
    #저장 경로 확인
...
btn_end = Button(frame_run,padx=5, pady=5, text="닫기", width=12, command=root.quit)
